...
export function useTripFlowLayer({ filters, focusedStationId, tripDirection = 'all' }) {
    // Only station coordinates are needed here, so this consumes the shared
    // station-availability data hook directly instead of the whole
    // infrastructure layer (which also fetches bike routes it doesn't need).
