 ...
 * @param {{ showBikeRoutes: boolean, hasTripFlowFocus: boolean, tripFlowBounds: Object|null }} options - Extra flags forwarded to layer-specific legend builders.
 * @returns {{ entries: Array, includeBikeRoutes?: boolean }} Legend descriptor consumed by MapLegend.
 */
export function legendFor(activeLayer, { showBikeRoutes, hasTripFlowFocus = false, tripFlowBounds = null }) {
    switch (activeLayer) {
        case "station_usage":
            return stationUsageLegend();
        case "trip_flow":
            return tripFlowLegend(hasTripFlowFocus, tripFlowBounds);
        case "infrastructure":
