...
In this section, we summarize the most significant data quality issues identified in our \href{../../technical-report/technical-report.pdf}{\underline{technical report}} and describe how we addressed them.
...
Citi Bike archives arrive in two packaging formats: yearly bundles containing monthly archives (legacy), and flat monthly files (modern).
...
The pipeline reconciles discrepancies between the GBFS live feed and Citi Bike historical trip archives through three main rules:
\begin{itemize}
...
First, the \textbf{data acquisition} step gathers the static datasets evaluated in our \href{../../technical-report/technical-report.pdf}{\underline{technical report}}: the trip archives, hourly weather, bike-route geometries, and a station metadata snapshot.
Each download is cached on disk with a freshness window, so repeated runs re-fetch only what has aged.
The step also derives what the sources omit, computing pairwise station distances from the metadata coordinates.
