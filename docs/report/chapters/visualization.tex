...
This structural repetition minimizes the cognitive load on users as they navigate between views.
    
\subsection{Header}
All four views are accessible via a fixed global header, enabling single-click navigation and persistent filtering. 
In alignment with our proposal, this architecture facilitates direct comparisons across views under a uniform filter set while mitigating the cognitive load associated with a single, densely packed interface. 
Modifying a filter triggers a data refetch and redraws all plots on the active view, with the updated filter state persisting seamlessly during route transitions.

The header incorporates two primary filter controls (Figure~\ref{fig:viz-filters}):
\begin{itemize}
