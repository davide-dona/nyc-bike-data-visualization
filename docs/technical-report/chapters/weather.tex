...
The API returns parallel hourly arrays derived from reanalysis models, that updates with a few days' lag, keeping the historical analysis stable.

\subsubsection{Original Structure}
The dataset includes several attributes, but we consider only the following main fields:
\begin{itemize}
  \item \textbf{Timestamp (time)}: hourly observation time, requested in the \texttt{America/New\_York} timezone so that it aligns with the trip timestamps.
...
\begin{figure}[H]
    ...
    \caption{Daily ridership, mean temperature, and total precipitation over the reference month, plotted on aligned date axes with individual scales. The dashed guide highlights 16 February, which recorded the highest rainfall and the lowest ridership.}
    \label{fig:rides_vs_weather}
\end{figure}

\subsection{Data Quality}
The archive is complete, with an hourly series covering the entire year and no missing values in any variable.
