...
\begin{comment}
    ...
    7) Also, i think that all the references to preprocessing / filtering should be removed, as they are not relevant to the technical report. It should only highlight the data quality issues, not how we fixed them.
...
Then, these individual trips were then aggregated to derive:
\begin{itemize} 
...
Despite these structural discrepancies, the historical data remains still valuable.
...
Finally, some trips report impossible distances ($11{,}000$~km as an example), due to corrupted station coordinates, and are filtered out using a maximum-distance threshold.
